...
Reglas de la tabla:
- **Adaptá las filas al change real**: sin migración → quitá el `migrate <N>` de la fila de CI; sin caso e2e real → quitá esa fila; PR vs merge-a-feature → ajustá la fila de destino; proyecto sin CI → fila `CI` con `⚠️ N/A (proyecto sin pipeline)`.
